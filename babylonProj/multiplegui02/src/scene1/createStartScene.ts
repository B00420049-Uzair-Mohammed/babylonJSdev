...
import {
  ...
  texture.diffuseTexture = new Texture("./src/assets/textures/wood.jpg", scene);
...
function createArcRotateCamera(scene: Scene) {
  ...
  export default function createStartScene(engine: Engine) {
    ...
    let that: SceneData = { scene: new Scene(engine) } as SceneData;
    ...
    that.extrusion = createExtrusion(that.scene);
    that.octahedron = createOctahedron(that.scene);
    that.plane = createPlane(that.scene);
    that.plane2 = createPlane2(that.scene);
